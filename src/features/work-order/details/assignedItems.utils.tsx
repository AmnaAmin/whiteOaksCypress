    ...
    <Box>
      ...
      {!value ? (
        <Button
          ...
          variant="outline"
          leftIcon={<BiUpload color="brand.500" />}
          display="flex"
        >
          {t(`${WORK_ORDER}.${label}`)}
        </Button>
      ) : (
        <Box color="brand.500" border="1px solid #022c7c" borderRadius="4px" fontSize="14px">
          <HStack spacing="5px" h="31px" padding="10px" align="center">
            <Text as="span" maxW="70px" isTruncated title="something">
              {value}
            </Text>
            <MdOutlineCancel color="brand.500" cursor="pointer" onClick={onFileClear} />
          </HStack>
        </Box>
      )}
    </Box>
