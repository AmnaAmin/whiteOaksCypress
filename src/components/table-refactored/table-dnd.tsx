...
import { Column, Table as TableType, TableOptions, flexRender, Row } from '@tanstack/react-table'

import { Table as ChakraTable, Thead, Tbody, Tr, Th, Td, Text, Flex, Stack, Box, Tfoot, Button } from '@chakra-ui/react'
...
const DragDropEnabledRows = ({ handleOnDrag, getRowModel, onRowClick, onRightClick, getColumnMaxMinWidths }) => {
  return (
    <DndProvider backend={HTML5Backend}>
      <Tbody>
        {getRowModel().rows.map?.(row => (
          <DraggableRow key={row.id} row={row} reorderRow={handleOnDrag} />
        ))}
      </Tbody>
    </DndProvider>
  )
