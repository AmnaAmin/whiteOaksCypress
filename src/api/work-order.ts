...
export const defaultValuesPayment = (workOrder, paymentsTerms) => {
  const defaultValues = {
    ...
    invoiceAmount: currencyFormatter(workOrder?.invoiceAmount)?.replace('$', '') || '',
    clientOriginalApprovedAmount: currencyFormatter(workOrder?.clientOriginalApprovedAmount)?.replace('$', '') || '',
    clientApprovedAmount: currencyFormatter(workOrder?.clientApprovedAmount)?.replace('$', '') || '',
    partialPayment: 0,
