...
import { dateFormat, dateFormatNew } from 'utils/date-time-utils'
...
export const createInvoicePdfNew = ({
  doc,
  invoiceVals,
  finalSowLineItems,
  receivedLineItems,
  ...
  img.onload = function () {
    ...
    doc.setFont(summaryFont, 'bold')
    doc.text('To:', x + 5, y + 20)
    doc.setFont(summaryFont, 'normal')

    doc.setFont(summaryFont, 'normal')
    doc.text('Accounts Payable', x + 30, y + 20)

    doc.setFont(summaryFont, 'normal')
    doc.text(projectData?.clientName ?? '', x + 30, y + 24)

    // doc.setFont(summaryFont, 'normal')
    // doc.text(projectData?.streetAddress ?? '', x + 30, y + 24)
    // doc.setFont(summaryFont, 'normal')
    // doc.text(projectData?.stateName + ', ' + projectData?.state + ' ' + projectData?.zipCode, x + 30, y + 28)
    ...
    if (finalSowLineItems.length > 1) {
      invoiceTotal = finalSowLineItems?.map(p => p?.amount).reduce((prev, curr) => Number(prev) + Number(curr), 0)
    } else {
      invoiceTotal = finalSowLineItems[0]?.amount
    }

    const summaryInfo = [
      {
        title: 'Total',
        value: currencyFormatter(invoiceTotal),
      },
    ]
    ...
    const receivableNoDataCheck = receivedLineItems.length > 0
    ...
    let sizeVar = receivableNoDataCheck ? 60 : 45
    let rectYY = tableEndsY + sizeVar //45 for 2 invoices
    if (doc.internal.pageSize.getHeight() - tableEndsY < 30) {
      doc.addPage()
      rectY = 20
    }
    ...
    let recievableInvoice
    if (receivedLineItems.length > 1) {
      recievableInvoice = receivedLineItems?.map(p => p?.amount).reduce((prev, curr) => prev + curr, 0)
    } else {
      recievableInvoice = receivedLineItems[0]?.amount
    }

    const summaryInfo1 = [
      {
        title: 'Total',
        value: receivableNoDataCheck ? currencyFormatter(recievableInvoice) : currencyFormatter(0),
      },
      {
        title: 'Invoice Amount',
        value: receivableNoDataCheck
          ? currencyFormatter(invoiceTotal - recievableInvoice)
