  ...
  const { data: transactions, ...rest } = useQuery<Array<TransactionType>>(
    ...
    async () => {
      const response = await client(
        `change-orders?projectId.equals=${projectId}&sort=modifiedDate,asc`,
        {}
      );
...
export const useProjectWorkOrdersWithChangeOrders = (projectId?: string) => {
  ...
  const { data: changeOrders, ...rest } = useQuery<Array<ProjectWorkOrder>>(
    ...
    async () => {
      const response = await client(
        `project/${projectId}/workordersWithChangeOrders`,
        {}
      );
...
export const useProjectWorkOrders = (projectId?: string) => {
  ...
  if (isAdmin) {
    againstOptions = workOrderOptions
      ? [AGAINST_DEFAULT_OPTION, ...workOrderOptions]
      : [AGAINST_DEFAULT_OPTION];
  } else {
...
export const useWorkOrderChangeOrders = (workOrderId?: string) => {
  ...
  const { data: changeOrders, ...rest } = useQuery<Array<ProjectWorkOrder>>(
    ...
    async () => {
      const response = await client(
        `change-orders?parentWorkOrderId.equals=${workOrderId}`,
        {}
      );
...
export const parseChangeOrderAPIPayload = (
  formValues: FormValues,
  projectId?: string
): ChangeOrderPayload => {
  const expectedCompletionDate = dateISOFormat(
    formValues.expectedCompletionDate
  );
  const newExpectedCompletionDate = dateISOFormat(
    formValues.newExpectedCompletionDate
  );
...
export const parseTransactionToFormValues = (
  ...
  return {
    ...
    against:
      againstOptions.find(
        (option) => option.value === transaction.parentWorkOrderId
      ) ?? null,
    workOrder: null,
    changeOrder: null,
    expectedCompletionDate:
      dateFormat(transaction.parentWorkOrderExpectedCompletionDate as string) ??
      null,
    newExpectedCompletionDate: "",
