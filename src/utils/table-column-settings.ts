...
const generateSettingColumn = (
  options: generateSettingColumnProps
): TableColumnSetting => {
  return {
    ...options,
    id: options.order,
...
const sortTableColumnsBasedOnSettingColumnsOrder = (
  ...
  const getOrder = (accessor: any) =>
    settingColumns?.find((item) => item.colId === accessor)?.order;
...
export const useTableColumnSettings = (
  ...
  const { data: savedColumns, ...rest } = useQuery<TableColumnSetting[]>(
    "GetProjectColumn",
    async () => {
  ...
  const { columns: rezizedColumns, resizeElementRef } = useColumnWidthResize(
    filteredColumns
  );
  const tableColumns = sortTableColumnsBasedOnSettingColumnsOrder(
    settingColumns,
    rezizedColumns
  );
